...
openwrt_path="${make_path}/rootfs"
openwrt_rootfs_file="*rootfs.tar.gz"
amlogic_path="${make_path}/anemo"
...
download_depends() {
    ...
    if [[ -d "${openvfd_path}" ]]; then
        ...
        svn co ${depends_repo}/config/rootfs/usr/share/openvfd ${openvfd_path} --force
    fi
    # Download balethirq related files
    svn export ${depends_repo}/config/rootfs/usr/sbin/balethirq.pl ${configfiles_path}/rootfs/usr/sbin --force
    svn export ${depends_repo}/config/rootfs/etc/balance_irq ${configfiles_path}/rootfs/etc --force
